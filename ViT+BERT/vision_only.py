...
CONFIG = {
    ...
    'checkpoint_dir': '/work/classtmp/rohail03/visiononly/checkpoints',
    'results_dir': './visonlyresults',
}
...
class VisionOnlyDataset(Dataset):
    def __init__(self, data_list, vision_processor, config, weights=None):
        self.data = data_list
...
class VisionOnlyRegressor(nn.Module):
    def __init__(self, config):
        ...
        print(f"Loading CLIP vision encoder: {config['vision_encoder']}...")
    ...
    def _init_weights(self):
        for m in [self.vision_proj, self.regressor]:
    ...
    def forward(self, pixel_values):
        ...
        vision_pooled = vision_feats.mean(dim=1)
        prediction = self.regressor(vision_pooled)
        return prediction
...
vision_processor = CLIPImageProcessor.from_pretrained(CONFIG['vision_encoder'])

print("✓ Processors loaded!")

print("Creating datasets...")
train_dataset = VisionOnlyDataset(train_data, vision_processor, CONFIG)
val_dataset = VisionOnlyDataset(val_data, vision_processor, CONFIG, weights=CONFIG['virality_weights'])
test_dataset = VisionOnlyDataset(test_data, vision_processor, CONFIG, weights=CONFIG['virality_weights'])
...
model = VisionOnlyRegressor(CONFIG).to(CONFIG['device'])
...
print("\n" + "="*80)
print("STARTING TRAINING - VISION ONLY ABLATION")
print("="*80 + "\n")
...
for epoch in range(CONFIG['num_epochs']):
    ...
    if val_metrics['r2'] > best_val_r2:
        ...
        torch.save(checkpoint, Path(CONFIG['checkpoint_dir']) / 'best_model_vision_only.pth')
...
axes[0].set_title('Training and Validation Loss (Vision Only)')
axes[0].legend()
...
plt.tight_layout()
plt.savefig(Path(CONFIG['results_dir']) / 'training_curves_vision_only.png', dpi=300)
plt.show()
...
checkpoint = torch.load(Path(CONFIG['checkpoint_dir']) / 'best_model_vision_only.pth')
...
axes[0].set_title(f'Test Set Predictions - Vision Only (R²={test_metrics["r2"]:.3f})')
axes[0].legend()
axes[0].grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig(Path(CONFIG['results_dir']) / 'test_predictions_vision_only.png', dpi=300)
plt.show()

print("\n✓ All done! Vision-only ablation complete.")
